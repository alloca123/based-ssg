...
static int 	docomment(const char *begin, const char *end, int newblock);	/* Parser for
										 * html-comments */
...
static int 	dolineprefix(const char *begin, const char *end, int newblock);	/* Parser for line
										 * prefix tags */
static int 	dolink(const char *begin, const char *end, int newblock);	/* Parser for links and
										 * images */
...
static int 	doreplace(const char *begin, const char *end, int newblock);	/* Parser for simple
										 * replaces */
static int 	doshortlink(const char *begin, const char *end, int newblock);	/* Parser for links and
										 * images */
static int 	dosurround(const char *begin, const char *end, int newblock);	/* Parser for
										 * surrounding tags */
static int 	dounderline(const char *begin, const char *end, int newblock);	/* Parser for underline
										 * tags */
static void    *ereallocz(void *p, size_t size);
static void 	hprint(const char *begin, const char *end);	/* escapes HTML and
								 * prints it to output */
static void 	process(const char *begin, const char *end, int isblock);	/* Processes range
										 * between begin and
										 * end. */
...
dohtml(const char *begin, const char *end, int newblock)
{
	...
	while ((p = strstr(p, "</")) && p < end) {
		...
		if (strncmp(p, tag, tagend - tag) == 0 && p[tagend - tag] == '>') {
			p++;
			fwrite(begin, sizeof(char), p - begin + tagend - tag + 1, output);
			return p - begin + tagend - tag + 1;
		}
	}
	p = strchr(tagend, '>');
	if (p) {
		fwrite(begin, sizeof(char), p - begin + 2, output);
		return p - begin + 2;
	} else
		return 0;
}
...
dolineprefix(const char *begin, const char *end, int newblock)
{
	...
	for (i = 0; i < LENGTH(lineprefix); i++) {
		...
		fputs(lineprefix[i].after, output);
		fputc('\n', output);
		free(buffer);
		return -(p - begin);
	}
...
int
smu_convert(FILE * out, FILE * source, int supresshtml)
{
	char           *buffer = NULL;
	int 		s        , i;
